...
class _RegistroRecorridoScreenState extends State<RegistroRecorridoScreen> {
  ...
  DateTime? fecha;
  DateTime? filtroFecha; // Fecha de filtro
  TimeOfDay? hora;
  String? destino;
  int? pasajeros;
  bool? llevaEquipaje;
  ...
  List<Recorrido> recorridos = [
    Recorrido(tipoRecorrido: 'Entrada', fecha: DateTime(2025, 8, 13), destino: 'Quito', pasajeros: 4),
    Recorrido(tipoRecorrido: 'Salida', fecha: DateTime(2025, 8, 14), destino: 'Guayaquil', pasajeros: 2),
    Recorrido(tipoRecorrido: 'Cancelada - Pagada', fecha: DateTime(2025, 8, 12), destino: 'Latacunga', pasajeros: 3),
  ];

  List<Recorrido> recorridosFiltrados = [];
  ...
  void filtrarPorFecha() {
    if (filtroFecha != null) {
      recorridosFiltrados = recorridos.where((recorrido) {
        return DateFormat('yyyy-MM-dd').format(recorrido.fecha) == DateFormat('yyyy-MM-dd').format(filtroFecha!);
      }).toList();
    } else {
      recorridosFiltrados = List.from(recorridos); // Si no hay filtro, mostrar todos
    }
  }
  ...
  Widget buildFechaFiltro() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Filtrar por Fecha", style: TextStyle(color: Colors.white)),
        const SizedBox(height: 8),
        GestureDetector(
          onTap: () async {
            final DateTime? picked = await showDatePicker(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    filtroFecha != null
                        ? DateFormat('dd/MM/yyyy').format(filtroFecha!)
        ...
        ElevatedButton(
          onPressed: () {
            setState(() {
              filtrarPorFecha();  // Llamamos a la función que filtra los recorridos
            });
          },
          child: const Text("Filtrar Recorridos"),
          style: ElevatedButton.styleFrom(
            backgroundColor: Colors.cyanAccent, // Color de fondo
  ...
  Widget buildRecorridosList() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          title: Text('Tipo: ${recorrido.tipoRecorrido}'),
          subtitle: Text('Fecha: ${DateFormat('dd/MM/yyyy').format(recorrido.fecha)}\nDestino: ${recorrido.destino}\nPasajeros: ${recorrido.pasajeros}'),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 30),
              buildFechaFiltro(),  // Filtro por fecha
              const SizedBox(height: 20),
              buildRecorridosList(),  // Lista de recorridos filtrados
            ],
          ),
        ),
      ),
...
class Recorrido {
  final String tipoRecorrido;
  final DateTime fecha;
  final String destino;
  final int pasajeros;

  Recorrido({
    required this.tipoRecorrido,
    required this.fecha,
